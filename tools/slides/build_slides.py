...
# ---- book settings ---------------------------------------------------------
BOOK, BOOK_EN, BOOK_NO = "ផែនទីវិទ្យា", "Cartography", "សៀវភៅទី១"
...
def deck(n, md):
    ...
    pending_sim = None
    theory_like = lambda s: ("ទ្រឹស្ដី" in s) or ("សិក្ខាសាលា" in s)

    for kind, p in blocks(md):
        if kind == "h2":
            sec, sub = p, ""
            if theory_like(p):
                label = "ទ្រឹស្ដីស្នូល" if "ទ្រឹស្ដី" in p else re.sub(r"^[\d០-៩.]+\s*", "", p)
                slides.append(S("divider", label, "", cls="theory" if "ទ្រឹស្ដី" in p else "workshop"))
            continue
        if kind == "h3":
        ...
        if kind == "list":
            ...
            if where == "scen": data["scen"].append(p)
                ...
                if last and last["kind"] == "idea" and last["title"] == sub and len(last["notes"]) + len(p) < 520 and last["notes"].count("\n") < 1:
...
def render(n, title, slides):
    ...
    for k, s in enumerate(slides, 1):
        ...
        if s["kind"] == "title":
            ...
            parts.append(f'<section class="slide {s["kind"]} {s["cls"]}"><h2>{inline(s["title"])}</h2>{s["body"]}{notes}{foot}</section>')
